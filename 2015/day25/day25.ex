...
defmodule Day25 do
  ...
  with the previous value, 20151125. Multiply it by 252533 to get 5088824049625.
  ...
  few numbers; for example, you're missing the one at 7,1 that would come before 6,2.
  ...
  def previous({1, 1}), do: nil
  def previous({1, y}), do: {y-1, 1}
  def previous({x, y}), do: {x-1, y+1}

  def code_at({1, 1}), do: 20151125
  def code_at(coords), do: coords |> previous |> code_at |> calc_next

  defp calc_next(x), do: rem(x * 252533, 33554393)
...
defmodule Day25Test do
  ...
  test "previous" do
    assert nil == Day25.previous({1, 1})
    assert {1, 1} == Day25.previous({1, 2})
    assert {1, 2} == Day25.previous({2, 1})
    assert {3, 3} == Day25.previous({4, 2})
    assert {2, 4} == Day25.previous({3, 3})
    assert {5, 2} == Day25.previous({6, 1})
    assert {5, 2} == Day25.previous({6, 1})
  end

  test "code_at" do
    assert 20151125 == Day25.code_at({1, 1})
    assert 31916031 == Day25.code_at({1, 2})
    assert 18749137 == Day25.code_at({2, 1})
    assert 6899651 == Day25.code_at({4, 5})
  end
  ...
  test "part 1" do
    ...
    IO.puts "part 1: #{Day25.code_at({col, row})}"
  end
end
