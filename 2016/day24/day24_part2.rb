...
def crawl(maze, x, y, locations, steps = 0)
  if locations.size == 1
    loc_x, loc_y = locations.first
    path_to_location = maze.shortest_path(x, y, loc_x, loc_y)
    path_to_start = maze.shortest_path(loc_x, loc_y, maze.start[0], maze.start[1])
    return steps + path_to_location.size - 1 + path_to_start.size - 1
  else
